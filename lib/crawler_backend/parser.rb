  ...
  class Parser
    ...
    def is_proxy(line)
      ...
      if line =~ /\d*\.\d*\.\d*\.\d*:(\d+)(\d+)(\d+)(\d+)|\d*\.\d*\.\d*\.\d*:(\d+)(\d+)/
        regex = 1
        proxy[:server_ip] = line.split(":")[0].chomp()
        ...
        regex = 2
        proxy[:server_ip] = line[/\b(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\b/].to_s.strip.chomp()
      ...
      if proxy[:server_ip] and proxy[:port]
        return proxy
      else
        return false
      end
